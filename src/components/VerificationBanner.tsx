...
export function VerificationBanner({ email }: { email: string }) {
  ...
  return (
    <aside
      aria-label="Vérification d'email requise"
      className="bg-info-bg border border-info-border rounded-lg p-3 sm:p-4 mb-4 flex items-start sm:items-center justify-between gap-3 flex-wrap"
    >
      <p className="text-sm text-info-text">
        <span aria-hidden>⚠️</span> Email{" "}
        <span className="font-medium">{email}</span> non vérifié. Cliquez sur le
        lien dans l'email reçu à l'inscription pour activer votre compte.
      </p>
      <button
        type="button"
